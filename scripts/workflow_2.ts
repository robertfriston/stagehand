...
const userCredentials = {
  firstname: "test",
  lastname: "user2", // Unique last name for this workflow
  email: "test.user2@jobenvy.co.uk", // Unique email
...
async function run() {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const nameWithTimestamp = `${userCredentials.firstname} W2 ${environmentLabel} [${timestamp}]`;
    ...
    try {
      ...
      console.warn(
        "Workflow 2: Chatbot interaction 1 failed.",
        error instanceof Error ? error.message : error,
      );
      ...
      try {
        ...
        console.warn(
          "Workflow 2: Could not click Back after Chatbot 1 failure.",
    ...
    try {
      ...
      console.warn(
        "Workflow 2: Chatbot interaction 2 failed.",
        error instanceof Error ? error.message : error,
      );
      try {
        await page.getByText("Back").click();
        ...
        console.warn(
          "Workflow 2: Could not click Back after Chatbot 2 failure.",
          backError instanceof Error ? backError.message : backError,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "Workflow 2: Error during Phase Two.",
        error instanceof Error ? error.message : error,
      );
      ...
      try {
        ...
        console.error(
          "Workflow 2: Failed to navigate home after Phase Two error.",
