// filepath: /Users/jobenvy/Documents/UTOPIA/stagehand/scripts/workflow_3.ts
// Copy of single_test.ts for workflow 3
...
async function run() {
  console.log("Starting Workflow 3"); // Added identifier
  ...
  console.log(`Workflow 3 Navigating to: ${targetUrl}`);
  await page.goto(targetUrl);

  // Loop through the workflow twice
  for (let i = 0; i < 2; i++) {
    console.log(`Workflow 3: Starting iteration ${i + 1}`);
    ...
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("lastNameString").fill("user3"); // Modified lastname
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("email").click();
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("email").fill("test.user3@jobenvy.co.uk"); // Modified email for uniqueness
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("username").click();
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("username").fill("testuser3"); // Modified username
    await page.waitForTimeout(defaultTimeout);
    ...
    try {
      ...
      console.log("Workflow 3: Clicked login tab/button successfully.");
    } catch (error) {
      console.warn(
    ...
    const nameWithTimestamp = `Test W3 ${environmentLabel} [${timestamp}]`; // Construct the name
    ...
    page.once("dialog", (dialog) => {
      console.log(`Workflow 3 Dialog message: ${dialog.message()}`);
    ...
    console.log(
      "Workflow 3: Attempting login/signup again after delete action",
    );
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("signin").click();
    await page.waitForTimeout(defaultTimeout);
    ...
    try {
      ...
      await page.getByTestId("firstNamesString").fill("test");
      await page.getByTestId("lastNameString").fill("user3"); // Unique last name
      await page.getByTestId("email").fill("test.user3@jobenvy.co.uk"); // Unique email
      await page.getByTestId("username").fill("testuser3"); // Unique username
      await page.getByTestId("password").fill("testuser");
      await page.getByTestId("agree").click();
      await page.getByRole("button", { name: "Sign Up" }).click();
      await page.waitForTimeout(defaultTimeout);
    ...
    await page.getByTestId("username").click();
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("username").fill("testuser3"); // Use unique username
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("password").click();
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("password").fill("testuser");
    ...
    await page.waitForTimeout(defaultTimeout);
    console.log("Workflow 3: Logged in successfully.");
    ...
    try {
      ...
      await page
        ...
        .fill("Workflow 3: hi - help me update my master resume");
    ...
    try {
      ...
      await page
        .getByTestId("RNE__Input__text-input")
        .fill("Workflow 3: what is the job situation like in Brighton UK?");
      await page.waitForTimeout(5000);
      await page.getByText("").click();
    ...
    console.log("Workflow 3: Starting Phase Two");
    ...
    try {
      ...
      await page.waitForTimeout(defaultTimeout);
      await page.getByTestId("addNew").fill("Test Resume W3"); // Unique name
      await page.waitForTimeout(defaultTimeout);
      ...
      await page.getByTestId("done").getByTestId("iconIcon").click();
      await page.waitForTimeout(defaultTimeout);
      await page.locator("span").filter({ hasText: "Test Resume W3" }).click(); // Use unique name
      await page.waitForTimeout(defaultTimeout);
      await page
        .locator("div")
      ...
      await page.waitForTimeout(defaultTimeout);
      await page.getByTestId("jobTitle").fill("Test Job Title W3"); // Unique title
      await page.waitForTimeout(defaultTimeout);
      ...
      await page.waitForTimeout(defaultTimeout);
      await page.getByTestId("summary").fill("Test Summary W3"); // Unique summary
      await page.waitForTimeout(defaultTimeout);
      await page.getByTestId("headerContainer").getByTestId("iconIcon").click();
      await page.waitForTimeout(defaultTimeout);
      await page.getByText("Back").click();
    ...
    console.log("Workflow 3: Logging back in for next step/iteration");
    await page.getByTestId("username").click();
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("username").fill("testuser3"); // Use unique username
    await page.waitForTimeout(defaultTimeout);
    ...
    await page.getByTestId("seek").click();
    await page.waitForTimeout(defaultTimeout);
    console.log("Workflow 3: Logged back in.");
    ...
    if (i === 0) {
      console.log(
        "Workflow 3: First iteration complete. Preparing for second iteration (still on LOCAL).",
      );
      ...
      console.log("Workflow 3: Second iteration complete.");
    }
  }

  console.log("Workflow 3 completed twice. Closing.");
  // await page.pause(); // Remove pause for automated runs
  await sh.close();
}

run().catch((error) => {
  console.error("Workflow 3 failed:", error);
