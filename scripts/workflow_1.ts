...
// Copy of single_test.ts for workflow 1
...
async function run() {
  ...
  console.log(`Workflow 1 Navigating to: ${targetUrl}`);
  await page.goto(targetUrl);

  // Loop through the workflow twice
  for (let i = 0; i < 2; i++) {
    console.log(`Workflow 1: Starting iteration ${i + 1}`);
    ...
    try {
      ...
      console.log("Workflow 1: Clicked login tab/button successfully.");
    } catch (error) {
      console.warn(
    ...
    page.once("dialog", (dialog) => {
      console.log(`Workflow 1 Dialog message: ${dialog.message()}`);
    ...
    console.log(
      "Workflow 1: Attempting login/signup again after delete action",
    );
    await page.waitForTimeout(defaultTimeout);
    await page.getByTestId("signin").click();
    await page.waitForTimeout(defaultTimeout);
    ...
    await page.waitForTimeout(defaultTimeout);
    console.log("Workflow 1: Logged in successfully.");
    ...
    try {
      ...
      await page
        ...
        .fill("Workflow 1: hi - help me update my master resume");
    ...
    try {
      ...
      await page
        .getByTestId("RNE__Input__text-input")
        .fill("Workflow 1: what is the job situation like in Brighton UK?");
      await page.waitForTimeout(5000);
      await page.getByText("").click();
